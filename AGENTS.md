...
- **visual payload architecture** — refresh only when the **VPA topology** (`docs/visual-payload-architecture-v2.md`) changes; **not** coupled to plateau milestones (it is an illustrative architecture companion to that doc — topology-coupled, not plateau/repo-state-coupled). Two render modes off one `*.source.js`: edit the source data, then re-render **both** the static shell (the article PNG) and validate the interactive shell. Its committed PNG (`asset-pipeline-ASK_visual-payload-architecture.png`, chrome-free static render on the **dark** gradient, embedded in `docs/visual-payload-architecture-v2.md`) is the same deliberate exception; HTML + source data remain canonical, PNG supplemental. (Repo embed = dark render; the light render is used for the Substack figure — both off one source model, one per surface.) The `diagram-static-FLOW` scaffold (engine / `diagrams.css` / `export-png.js`) is vendored by reference from `design-system-ASK` (pinned `98d6074`) and **never forked locally** — re-vendor on upstream change.

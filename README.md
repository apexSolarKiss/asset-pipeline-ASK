...
- [`docs/collection-mode-operational-test-setup-v1.md`](docs/collection-mode-operational-test-setup-v1.md): Phase 1 setup of the first cross-mode operational test — created 3 net-new output_slots on PKT-COLL-001 (HERO_GROUP, HERO_GROUP_ALT, DETAIL_ADJACENCY) configured to pressure-test whether the slot-input plurality strain becomes an operational blocker or remains content-discipline-only; runs in the existing SKU-driven base to test mode-independence directly; Phase 2 (generation + curation + findings) deferred

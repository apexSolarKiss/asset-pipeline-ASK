...
- **Trace-carrier design decision surface** — would propose specific carrier shapes for the held trace dimensions (`articulation_state`; `derivation_basis` with the five-value candidate sharpened by PR #250; `fallback_chain_position`; `contradiction_log`; negative-space trace) and read against PR #250's reconstruction-failure points. Newly named as a candidate next-pressure surface by PR #250 Section 8. Not authorized.
- **Real multi-operator continuity-break test** — would absorb PR #250's simulated continuity-break test as substrate and either earn or deflate its reconstruction-failure findings with concrete other-operator reconstruction attempts. Newly named as the natural successor to PR #250 in its Self-Superseding Clause. Not authorized.

...
- **visual payload architecture** — refresh only when the **VPA topology** (`docs/visual-payload-architecture-v2.md`) changes; **not** coupled to plateau milestones (it depicts the visual-payload doctrine, not repo state). Two render modes off one `*.source.js`: edit the source data, then re-render **both** the static shell (the article PNG) and validate the interactive shell. Its committed PNG (`asset-pipeline-ASK_visual-payload-architecture.png`, chrome-free static render on the light gradient, embedded in `docs/visual-payload-architecture-v2.md`) is the same deliberate exception; HTML + source data remain canonical, PNG supplemental. The `diagram-static-FLOW` scaffold (engine / `diagrams.css` / `export-png.js`) is vendored by reference from `design-system-ASK` (landed `c87aa0d`) and **never forked locally** — re-vendor on upstream change.

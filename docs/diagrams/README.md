...
asset-pipeline-ASK_visual-payload-architecture.png               committed static render (chrome-free, light gradient field) — embedded in docs/visual-payload-architecture-v2.md
...
export-png.js                                   3840×2880 PNG export (page-scale chrome; viewBox-origin + landscape-fit corrections)
...
This folder hosts four **Class A** static diagram patterns inherited by reference from `design-system-ASK`: `diagram-static-H` (horizontal cascade), `diagram-static-V` (vertical centered spine; delivered at merge `1c565e2`), `diagram-static-SEQ` (left-aligned arrowed sequence; graduated upstream at `28aeb2d` and re-vendored here), and `diagram-static-FLOW` (convergence flow with two render modes; graduated upstream from AP and re-vendored here at `c87aa0d`). `diagrams.css` and `export-png.js` are **byte-identical** across all four patterns, so this folder keeps **one shared copy** rather than vendoring each — a deliberate, documented choice. (Vendoring FLOW updated that shared copy to `c87aa0d`: the FLOW classes, the DS-wide light-mode line-color correction — white → dark ASK ink on the light gradient, applied across all four patterns by design — and the two `export-png.js` corrections for centered/negative-origin and landscape content. Both are backward-compatible; the portrait H/V/SEQ diagrams render unchanged except the intended darker light-mode lines.) The four layout engines (`diagrams-static-H-engine.js` / `-V-` / `-SEQ-` / `-FLOW-`) are scaffold-owned and not edited locally; they are re-vendored by reference, never forked.
...
The **visual payload architecture** (`diagram-static-FLOW`) is a third class again — an illustrative **doctrine/architecture** diagram of the *convergence flow* a commercial image's visual decisions move through: a reference carrier qualifying a normative-source field, the field converging into a resolved slot specification, realization (candidate image ‖ set relations) reconverging into conformance, then selection → governance → governed asset, with a return loop where a governed output can later re-enter as a reference carrier (basis: [`docs/visual-payload-architecture-v2.md`](../visual-payload-architecture-v2.md)). It is the first consumer of the `diagram-static-FLOW` scaffold AP graduated upstream and re-vendored here at `c87aa0d`. **Two render modes off one shared source model:** the chrome-free **static** shell (`…_visual-payload-architecture.html`) is the article target (legible without interaction); the full-chrome **interactive** shell (`…_visual-payload-architecture.interactive.html`) is the repo-native explanatory artifact — identical topology plus a hover/click side panel carrying each node's definition / example / "not-this" note. A committed PNG (`asset-pipeline-ASK_visual-payload-architecture.png`, chrome-free static render on the light gradient field) is kept alongside for direct markdown/GitHub embedding (same deliberate exception as the discretion chain); it is embedded in [`docs/visual-payload-architecture-v2.md`](../visual-payload-architecture-v2.md). The HTML + source data remain **canonical**; the PNG is a supplemental raster. Because it depicts the visual-payload doctrine rather than repo/architecture state, it refreshes when the **VPA topology** changes, not on the milestone cadence. To refresh the PNG, re-render the static shell from the bundle and re-export.

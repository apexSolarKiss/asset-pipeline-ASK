# Placeholder Furniture Constraint Layering Note

This note clarifies current constraint scope within the placeholder furniture SKU-driven context and remains provisional.

## Brand-Wide Constraints

Brand-wide constraints are the stable visual and behavioral constraints that apply across the placeholder brand as a whole. In the current furniture example, these include calm presentation, scene restraint, material-led clarity, proportion credibility, and the expectation that outputs stay usable for controlled ecommerce and catalog contexts.

## Category-Level Constraints

Category-level constraints apply across a broader grouping before class-specific details matter. In the current placeholder furniture context, these constraints hold across furniture generally: products should remain legible, environments should stay secondary to the product, and fidelity should remain high enough that materials, construction, and scale are not casually reinterpreted.

## Product-Class-Level Constraints

Product-class-level constraints apply to a class such as sofa or dining chair. These constraints shape what must remain readable for that class, which views matter most, and which distortions are especially risky. A sofa, for example, carries different legibility pressure around massing, arm profile, and seat depth than a dining chair does.

## SKU-Specific Facts Or Exceptions

SKU-specific facts or exceptions are product truths and narrow constraints tied to one SKU. These include finish, upholstery, seam placement, identifying details, and any other narrow fact that must remain faithful for that specific item rather than for the class as a whole.

## What Belongs Elsewhere

These constraint scopes are not the same as product context input, style reference input, or bounded creative discretion. Product context input carries the factual product data and references. Style reference input carries approved directional examples. Bounded creative discretion defines where choices are allowed inside the constraints. Other workflow modes may later require different layering behavior, but that is outside this first SKU-driven pass.

## Working Rule

The repo should currently treat these scopes as conceptual layers inside the SKU-driven placeholder furniture context without turning them into formal schema or inheritance logic yet.

...
- **Portability**: the durable-layer-above-tools claim now carries the first bounded portability evidence outside Airtable shape — a data-shape extraction plus one proven package instance re-expressed and walked to instantiation-plan depth (see Portability / Extractability below). This is re-expression and plan depth, not runtime machine-usability: no second real substrate has been instantiated and no runtime consumer exists.
...
**Claim boundary.** Earned: the data shape is portable, and one proven package instance is re-expressible outside Airtable and walkable to instantiation-plan depth — the first bounded portability evidence outside Airtable shape. Not earned / not authorized: no second real substrate has been instantiated (the package walks the instantiation plan; it does not build it); no runtime consumer reads a package instance; no reusable package grammar is authored; no schema / validator / orchestrator / structured IA model v3 is authorized.
...
- Second-substrate instantiation, package grammar, and runtime consumer. The portability legs pressure re-expression outside Airtable and walk to instantiation-plan depth; building a second real substrate, authoring a reusable package grammar, or wiring a runtime consumer that reads a package instance is not authorized here.

...
This repo's `AGENTS.md` is a **resolved carrier**: the shared execution protocol (below, resolved from `control-surface/protocol/AGENTS.shared.md`) + the applicable `advisor-project-surface` profile + the standing upstream-conformance grant + the asset-pipeline-ASK-local delta. The shared block, profile, and grant are machine-maintained — do not hand-edit between the markers.
...
SHARED_BLOCK_PIN: 645d01ee7a4973435185cd0cd587201a6406836c
PROFILES: [advisor-project-surface]
GRANT_FRAGMENT: standing-upstream-conformance-grant@645d01ee7a4973435185cd0cd587201a6406836c
...
**Runtime-capacity claims are not verified state.** An executor cannot verify its own remaining effective capacity from workload, conversation length, estimated token use, accumulated tool output, perceived complexity, or a subjective sense of having "spent itself." A runtime-emitted capacity readout **is** verified runtime state, but a readout alone establishes neither exhaustion nor a new gate, and neither a readout nor an estimate creates a stop condition, transfer recommendation, recap obligation, program interruption, or ASK decision. Where the runtime reduces context automatically — by compaction, summarization, or equivalent — or the operator can trigger that reduction, **ordinary context pressure is a continuation path**: continue the current authorized unit up to its configured review gate. Where **no** reduction path exists — the runtime declares no automatic reduction *and* no operator-triggerable reduction exists, a reduction path being presumed to exist wherever either is unknown — an objective readout becomes decision-relevant only where the runtime declares a fixed limit and the executor names the exact readout value, that declared limit, and an already-observed capacity failure of an operation required to reach the next gate. **A projection of remaining work is not a demonstration.** Then **stop at the current safe point**. Preserve state through an existing durable owner only where that write is already inside the authorized operation; otherwise report the stop, the exact failed operation, and the recovery requirement to ASK. **This rule creates no recap, registry, checkpoint-artifact, or other write authority.** An upcoming already-authorized checkpoint may be used only where it is reachable without retrying, bypassing, or depending on the operation that already failed; do not perform additional work merely to reach a preferable stopping place. The readout still manufactures no new transfer gate. **On capacity grounds specifically**, interrupt the unit before an existing configured gate only when the runtime emits an explicit blocking signal or a failure of its reduction, or the declared-fixed-limit case above is established on its own terms. **Propose transfer** only when the runtime emits an explicit blocking signal or a failure of its reduction; required material is demonstrably unavailable after the normal recovery path; a defect is named against an exact artifact, output, or reproducible observation under the bullets above, a self-assessment of one's own output quality not being such a defect; or ASK directs transfer. The fixed-limit case is **not** among them: it earns an orderly checkpoint, never a transfer. **Stop conditions declared elsewhere in this protocol are unaffected.** **Reduction is not assumed lossless.** Report a possible fidelity loss as the exact missing material, recovered from its durable owner — never as a capacity estimate. Freshness as method is unaffected: independent critique, reconstruction from durable truth, a deliberate successor after a completed execution context, and an ASK-directed transfer remain legitimate, and none of them is evidence that the current thread reached a limit. A freshness proposal names its method reason and must hold at the start of a fresh thread; one that would not be proposed there is a capacity claim wearing a method label.

**Any context-reduction path must preserve or re-establish, before further action:**
...
Report the declared review identity in the handoff. A shared-scratch review object is an ordinary named operator-scratch artifact — a visible surface under normal proposal/scratch discipline, **not** a private persistent surface governed by §Private-Memory Write Gate.

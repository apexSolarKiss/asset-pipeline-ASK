...
SHARED_BLOCK_PIN: ad88eb048a7c5ed81f0847490decdadd925bb223
PROFILES: [advisor-project-surface]
GRANT_FRAGMENT: standing-upstream-conformance-grant@ad88eb048a7c5ed81f0847490decdadd925bb223
...
**Review-window routing.** For repository changes, the normal advisor review object is the **pushed PR**. A pasted executor summary — including a report of `exact scoped diff ready for approval` — is **not** an implicit request for pre-PR advisor review, and an advisor must not begin constructing or requesting transport work on the strength of one. Do not construct a pre-PR advisor packet unless ASK explicitly requests advisor review before authorizing commit and push. This rule governs that exceptional path only; it adds no publication step to the ordinary terminal state, which stands as defined in §Exact Scoped Diff Gate.

**Pre-PR publication condition.** Where ASK explicitly requests pre-PR advisor review, the executor may not report the object as ready for advisor review until a named proposal-only exact patch or bounded review object exists in the index-mapped shared scratch **and** its exact path, baseline, byte size, and SHA-256 have been reported. A session-local path, a prose summary, or a digest without the object does not satisfy this condition.

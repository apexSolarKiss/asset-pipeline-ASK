...
- **source of intent ≠ decision owner ≠ authorized decision-maker.** Three distinct roles carry the chain: the **source of intent** supplies purpose, authority, and recourse; the **decision owner** owns the aperture, its closure, and answerability; the **authorized decision-maker** evaluates and binds locally within a delegated scope. They may coincide but need not — substrate-neutral on who may decide, strict on where answerability remains. A computational agent may occupy the third role only by the functional test above; current model sampling does not.

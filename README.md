...
- [`docs/collection-mode-operational-test-findings-v1.md`](docs/collection-mode-operational-test-findings-v1.md): **Verdict**: the existing schema carries collection-mode operational pressure cleanly without structural mutation. All 3 slots produced ratifiable governed assets via the prose-only workaround the cross-mode probe predicted would strain. Composite-anchor schema mutation is *less* earned by operational pressure now than after the paper probe. Surfaces 10 operational findings including: prose-only naming carries 1:N slot-product cardinality; prose can carry exclusion as well as inclusion; generation tooling does not reliably honor "all items fully in frame" for elongated constituents (documented limitation); cross-slot consistency for prose-only constituents requires verbatim repetition; family coherence holds across slots without arrangement preservation
